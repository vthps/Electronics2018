...
double get_velocity(void) {
    ...
    //retrieve raw data
    double raw_static_pressure = analogRead(STATIC_SENSOR); // add/subtract constants to zero pressure sensors
    double raw_dynamic_pressure = analogRead(DYNAMIC_SENSOR);

    //convert from raw data to volts (5V = 1024 raw data increments)
    double static_voltage = (raw_static_pressure/1024) * 5;
    double dynamic_voltage = (raw_dynamic_pressure/1024) *5;
